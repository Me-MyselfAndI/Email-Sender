...
class TextBoxSetup(GridLayout):
    ...
    class FieldTitle (Label):
        def __init__(self, text, is_long=False, **kwargs):
            super().__init__(**kwargs)
            self.text = text + " " + ("(short)" if not is_long else "(long)")
            self.background_color = (0.3, 0.3, 0.3, 1)
            self.size_hint = (1, None)
            self.height = 30

        def on_size(self, screen, parent):
    ...
    def __init__(self, name="", is_long=False, **kwargs):
        super().__init__(**kwargs)
        self.delete_button = self.DeleteButton (name, **kwargs)
        self.field_title = self.FieldTitle (name, is_long=is_long, **kwargs)
        self.setup_label_field = TextInput (size_hint=(1, None), height=30, text="Type the title of this field here")
        self.cols = 4
        self.spacing = 10
